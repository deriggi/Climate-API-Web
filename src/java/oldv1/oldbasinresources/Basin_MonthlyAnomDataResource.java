...
@Path("/basin/manom")
public class Basin_MonthlyAnomDataResource {

    private String numbersPattern = "[0-9]{1,}";
    private Pattern p = null;
    private Logger log = Logger.getLogger(Basin_MonthlyAnomDataResource.class.getName());
    ...
    @GET
    @Path("/{gcm}/{scenario}/{stat}/{fyear}/{tyear}/{wbhuc : (?i)\\d{1,6}(\\.\\w{1,5}|\\.\\w{1,5})?}")
    public Response getMonthlyDataBasin(
        ...
        return getMonthlyDataBasinDelegate(gcmName, scenarioName, statName, fyear, tyear, Integer.parseInt(identifier), mediaType);
    }

    private Response getMonthlyDataBasinDelegate(String gcmName,
            String scenarioName,
            String statName,
            int fyear,
            int tyear,
    ...
    private Response getAllMonthlyDataBasinDelegate(
        ...
        config.setAreaId(basinId);
        List<GcmDatum> configs = P_GcmConfigAreaDao.get().getAllGcmAreaData(config, false);
...
//            return getMonthlyEnsembleDelegate(scenarioName, statName, fyear, tyear, percentile, iso3, MediaType.APPLICATION_XML);
//        }

        return getMonthlyEnsembleDelegate(scenarioName, statName, fyear, tyear, percentile, Integer.parseInt(identifier), mediaType);

    }

    private Response getMonthlyEnsembleDelegate(String scenarioName, String statName, int fyear, int tyear, int percentile, int wbhuc, String format) {
        int basinId = BasinDao.get().getBasinIdFromCode(wbhuc);
        log.log(Level.INFO, "getting basin id is {0}", basinId);
        P_GcmStatsProperties ds = P_GcmStatsProperties.getInstance();
        P_GcmStatsProperties.climatestat stat = ds.getClimateStat(statName);
