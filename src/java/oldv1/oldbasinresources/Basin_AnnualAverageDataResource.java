...
@Path("/basin/annualavg")
public class Basin_AnnualAverageDataResource {

    private Logger log = Logger.getLogger(Basin_AnnualAverageDataResource.class.getName());
    ...
    public Response getAnnualDataBasin(
        ...
        return getAnnualDataBasinDelegate(gcmName, scenarioName, statName, fyear, tyear, Integer.parseInt(identifier), mediaType);
    }
    ...
    public Response getAllAnnualDataIso(
            @PathParam("scenario") String scenarioName,
            @PathParam("stat") String statName,
            @PathParam("fyear") int fyear,
            @PathParam("tyear") int tyear,
        ...
        return getAllAnnualDataBasinDelegate(scenarioName, statName, fyear, tyear, Integer.parseInt(identifier), mediaType);
    }

    private Response getAllAnnualDataBasinDelegate(
            String scenarioName,
            String statName,
            int fyear,
            int tyear,
    ...
    private Response getAnnualEnsembleDelegate(String scenarioName, String statName, int fyear, int tyear, int percentile, int wbhuc, String format) {
        int basinId = BasinDao.get().getBasinIdFromCode(wbhuc);
        log.log(Level.INFO, "getting country id is {0}", basinId);
        P_GcmStatsProperties ds = P_GcmStatsProperties.getInstance();
        P_GcmStatsProperties.climatestat stat = ds.getClimateStat(statName);
        P_GcmStatsProperties.scenario scenario = ds.getScenario(scenarioName);
